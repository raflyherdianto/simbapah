...
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            ...
            <form class="row g-3 mt-2" action="/dashboard/pelanggan/{{ $pelanggan->id }}" method="POST"
                enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="col-12">
                    <label for="kode_user" class="form-label fs-5">Kode User</label>
                    <input class="form-control" type="text" value="{{ old('kode_user', $pelanggan->kode_user) }}" name="kode_user"  aria-label="readonly input example" readonly>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('jenis_kelamin', $pelanggan->jenis_kelamin) }}">
                    @error('jenis_kelamin')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('kelurahan', $pelanggan->kelurahan) }}">
                    @error('kelurahan')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('rekening', $pelanggan->rekening) }}">
                    @error('rekening')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    <input type="hidden" name="oldImage" value="{{ $pelanggan->foto }}">
                    @if ($pelanggan->foto)
