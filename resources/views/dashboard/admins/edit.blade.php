...
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            ...
            <form class="row g-3 mt-2" action="/dashboard/admin/{{ $admin->id }}" method="POST"
                enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="col-12">
                    <label for="kode_user" class="form-label fs-5">Kode User</label>
                    <input class="form-control" type="text" value="{{ old('kode_user', $admin->kode_user) }}" name="kode_user"  aria-label="readonly input example" readonly>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('jenis_kelamin', $admin->jenis_kelamin) }}">
                    @error('jenis_kelamin')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('provinsi', $admin->provinsi) }}">
                    @error('provinsi')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('kelurahan', $admin->kelurahan) }}">
                    @error('kelurahan')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    value="{{ old('kode_pos', $admin->kode_pos) }}">
                    @error('kode_pos')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    value="{{ old('rekening', $admin->rekening) }}">
                    @error('rekening')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
